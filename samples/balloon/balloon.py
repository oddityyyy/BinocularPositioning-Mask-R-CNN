"""
Mask R-CNN
Train on the toy apple dataset and implement color splash effect.
...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 apple.py train --dataset=/path/to/apple/dataset --weights=last

    # Train a new model starting from ImageNet weights
    python3 apple.py train --dataset=/path/to/apple/dataset --weights=imagenet

    # Apply color splash to an image
    python3 apple.py splash --weights=/path/to/weights/file.h5 --image=<URL or path to file>

    # Apply color splash to video using the last weights you trained
    python3 apple.py splash --weights=last --video=<URL or path to file>
"""
...
class appleConfig(Config):
    ...
    NUM_CLASSES = 1 + 1  # Background + strip
...
class appleDataset(utils.Dataset):
    ...
    def load_apple(self, dataset_dir, subset):
        ...
        dataset_dir = 'D:\\maskrcnn\\Mask_RCNN-master\\apple'
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        """
        # If not a apple dataset image, delegate to parent class.
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Train Mask R-CNN to detect apples.')
    ...
    parser.add_argument('--weights', required=True,
                        metavar=r"D:\maskrcnn\mask_rcnn_coco.h5",
