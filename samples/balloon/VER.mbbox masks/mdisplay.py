...
BALLON_WEIGHTS_PATH = "G:\ZGA\maskrcnn\logs\apple20200430T1315\mask_rcnn_apple_0300.h5"
...
config.IMAGE_SHAPE = np.array([960, 1280, 3])
...
left = cv2.imread('D:/maskrcnn/apple/val/val/1.png')
left2=left
right = cv2.imread('D:/maskrcnn/apple/val/val/2.png')
right2=right
...
lbbox,mbbox=del_minus1_in_bbox(lbbox,mbbox)
print(lbbox)
print(rbbox)
print(mbbox)
print('final result')
rbbox2=[]
mbbox_rmasks_related_list=imgmatch.mrbbox_match(mbbox,rbbox)
...
class process(object):
    ...
    def test_masks_and_bbox(self):
        ...
        for i in range(int(len(self.lbbox)/4)):
            ...
            cv2.rectangle(self.left,(self.lbbox[flag],self.lbbox[flag+1]),(self.lbbox[flag+2],self.lbbox[flag+3]),self.color,2)
            ...
            cv2.rectangle(self.right,(self.mbbox[flag],self.mbbox[flag+1]),(self.mbbox[flag+2],self.mbbox[flag+3]),self.color,2)
            for j in range(w):
                for k in range(h):
    ...
    def find4pos(self):
        ...
        for k in range(int(len(lbbox)/4)):
            index=k*4
            ...
            for i in range(self.lbbox[index+3]-self.lbbox[index+1]):
                ...
                for j in range(self.lbbox[index+2]-self.lbbox[index]):
                    ...
                    if self.lmasks[self.lbbox[index+1]+i,self.lbbox[index]+j,k]==True and uflag==0:
                        self.Luboxlist.append([self.lbbox[index]+j,self.lbbox[index+1]+i])
                        uflag=1
                    if self.lmasks[self.lbbox[index+3]-i,self.lbbox[index+2]-j,k]==True and dflag==0:
                        self.Ldboxlist.append([self.lbbox[index+2]-j,self.lbbox[index+3]-i])
                        dflag=1
            cv2.line(self.left,tuple(self.Luboxlist[-1]),tuple(self.Ldboxlist[-1]),(0,0,255),3)
            for i in range(self.lbbox[index+2]-self.lbbox[index]):
                ...
                for j in range(self.lbbox[index+3]-self.lbbox[index+1]):
                    ...
                    if self.lmasks[self.lbbox[index+1]+j,self.lbbox[index]+i,k]==True and lflag==0:
                        self.Llboxlist.append([self.lbbox[index]+i,self.lbbox[index+1]+j])
                        lflag=1
                    if self.lmasks[self.lbbox[index+3]-j,self.lbbox[index+2]-i,k]==True and rflag==0:
                        self.Lrboxlist.append([self.lbbox[index+2]-i,self.lbbox[index+3]-j])
                        rflag=1
            cv2.line(self.left,tuple(self.Llboxlist[-1]),tuple(self.Lrboxlist[-1]),(0,255,0),3)
            ...
            for i in range(self.mbbox[index+3]-self.mbbox[index+1]):
                ...
                for j in range(self.mbbox[index+2]-self.mbbox[index]):
                    ...
                    if self.rmasks[self.mbbox[index+1]+i,self.mbbox[index]+j,self.mbbox_rmasks_related_list[k]]==True and uflag==0:
                        self.Ruboxlist.append([self.mbbox[index]+j,self.mbbox[index+1]+i])
                        uflag=1
                    if self.rmasks[self.mbbox[index+3]-i,self.mbbox[index+2]-j,self.mbbox_rmasks_related_list[k]]==True and dflag==0:
                        self.Rdboxlist.append([self.mbbox[index+2]-j,self.mbbox[index+3]-i])
                        dflag=1
            cv2.line(self.right,tuple(self.Ruboxlist[-1]),tuple(self.Rdboxlist[-1]),(0,0,255),3)
            for i in range(self.mbbox[index+2]-self.mbbox[index]):
                ...
                for j in range(self.mbbox[index+3]-self.mbbox[index+1]):
                    ...
                    if self.rmasks[self.mbbox[index+1]+j,self.mbbox[index]+i,self.mbbox_rmasks_related_list[k]]==True and lflag==0:
                        self.Rlboxlist.append([self.mbbox[index]+i,self.mbbox[index+1]+j])
                        lflag=1
                    if self.rmasks[self.mbbox[index+3]-j,self.mbbox[index+2]-i,self.mbbox_rmasks_related_list[k]]==True and rflag==0:
                        self.Rrboxlist.append([self.mbbox[index+2]-i,self.mbbox[index+3]-j])
                        rflag=1
            cv2.line(self.right,tuple(self.Rlboxlist[-1]),tuple(self.Rrboxlist[-1]),(0,255,0),3)
    ...
    def locate(self):
        ...
        for i in range(len(self.Lrboxlist)):
            self.uD.append(round(BF/abs(self.Luboxlist[i][0]-self.Ruboxlist[i][0]),2))
            self.dD.append(round(BF/abs(self.Ldboxlist[i][0]-self.Rdboxlist[i][0]),2))
            self.lD.append(round(BF/abs(self.Llboxlist[i][0]-self.Rlboxlist[i][0]),2))
            self.rD.append(round(BF/abs(self.Lrboxlist[i][0]-self.Rrboxlist[i][0]),2))
        print(str(self.uD),str(self.dD),str(self.lD),str(self.rD))
    def vis(self):
        for i in range(len(self.uD)):
            cv2.putText(self.left,str(self.uD[i]),tuple(self.Luboxlist[i]),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2)
            cv2.putText(self.left,str(self.dD[i]),tuple(self.Ldboxlist[i]),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2)
            cv2.putText(self.left,str(self.lD[i]),tuple(self.Llboxlist[i]),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2)
            cv2.putText(self.left,str(self.rD[i]),tuple(self.Lrboxlist[i]),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2)
    def save(self):
        cv2.imwrite('left4pos.png',self.left)
        cv2.imwrite('right4pos.png',self.right)
sample=process(left2,right2,lbbox,lmasks,mbbox,rmasks,mbbox_rmasks_related_list)
sample.find4pos()
sample.locate()
sample.vis()
sample.save()
