...
BALLON_WEIGHTS_PATH = "D:\maskrcnn\logs\apple20200430T1315\mask_rcnn_apple_0300.h5"
                ...
                for j in range(self.lbbox[index+2]-self.lbbox[index]):
                    ...
                    if self.lmasks[self.lbbox[index+1]+i, self.lbbox[index]+j, k] == True and uflag == 0:
                        self.Luboxlist.append(
                            [self.lbbox[index]+j-random.randint(0,3), self.lbbox[index+1]+i-random.randint(0,3)])
                        uflag = 1
                    if self.lmasks[self.lbbox[index+3]-i, self.lbbox[index+2]-j, k] == True and dflag == 0:
                        self.Ldboxlist.append(
                            [self.lbbox[index+2]-j+random.randint(0,3), self.lbbox[index+3]-i+random.randint(0,3)])
                        dflag = 1
                ...
                for j in range(self.lbbox[index+3]-self.lbbox[index+1]):
                    ...
                    if self.lmasks[self.lbbox[index+1]+j, self.lbbox[index]+i, k] == True and lflag == 0:
                        self.Llboxlist.append(
                            [self.lbbox[index]+i+random.randint(0,3), self.lbbox[index+1]+j+random.randint(0,3)])
                        lflag = 1
                    if self.lmasks[self.lbbox[index+3]-j, self.lbbox[index+2]-i, k] == True and rflag == 0:
                        self.Lrboxlist.append(
                            [self.lbbox[index+2]-i-random.randint(0,3), self.lbbox[index+3]-j-random.randint(0,3)])
                        rflag = 1
...
sample.avg()
sample.z_vis()
print('uD:'+str(sample.uD))
print('dD:'+str(sample.dD))
print('lD:'+str(sample.lD))
print('rD:'+str(sample.rD))

cv2.imshow('zvis',sample.left)
cv2.waitKey(0)
#sample.show_xyz()
# sample.point()
