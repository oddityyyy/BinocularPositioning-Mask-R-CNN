...
saveDir = r'D:\maskrcnn\Mask_RCNN-master\samples\balloon\locate_result'
ROOT_DIR = os.path.abspath("../../")
path = r'D:\maskrcnn\maskrcnn\apple+logsYYY\apple\train\resize'
folderList = os.listdir(path)
leftImLs=[]
leftImLs=([[path+'\\'+x, x] for x in os.listdir(path) if x.find('jpg') is not -1])
# input(cv2.imread(leftImLs[0][0]).shape)
...
config.IMAGE_SHAPE = np.array([1920, 1088, 3])  # 此处解决了输出mask与输入图片分辨率不同的问题
...
class process(object):
    def __init__(self, left=None, right=None, lbbox=None, lmasks=None, mbbox=None, rmasks=None, mbbox_rmasks_related_list=None, name=None):
        self.left = left
        self.right = right
        self.lbbox = lbbox
        self.lmasks = lmasks
    ...
    def reinit(self, left, right, lbbox, lmasks, mbbox, rmasks, mbbox_rmasks_related_list, name):
        self.left = left
        self.right = right
        self.lbbox = lbbox
        self.lmasks = lmasks
        ...
        for k in range(l):
            ...
            for i in range(self.mbbox[index + 3] - self.mbbox[index + 1]):
                ...
                for j in range(self.mbbox[index + 2] - self.mbbox[index]):
                    ...
                    if self.rmasks[self.mbbox[index + 1] + i, self.mbbox[index] + j, self.mbbox_rmasks_related_list[
                        k]] == True and uflag == 0:
                        self.Ruboxlist.append(
                            [self.mbbox[index] + j, self.mbbox[index + 1] + i])
                        uflag = 1
                    if self.rmasks[self.mbbox[index + 3] - i, self.mbbox[index + 2] - j, self.mbbox_rmasks_related_list[
                        k]] == True and dflag == 0:
            ...
            for i in range(self.mbbox[index + 2] - self.mbbox[index]):
                ...
                for j in range(self.mbbox[index + 3] - self.mbbox[index + 1]):
                    if rflag == 1 and lflag == 1:
                        break
                    if self.rmasks[self.Llboxlist[-1][-1], self.mbbox[index] + i, self.mbbox_rmasks_related_list[
                        k]] == True and lflag == 0:
                        self.Rlboxlist.append(
                            [self.mbbox[index] + i, self.Llboxlist[-1][-1]])
                        lflag = 1
                    if self.rmasks[self.Lrboxlist[-1][-1], self.mbbox[index + 2] - i, self.mbbox_rmasks_related_list[
                        k]] == True and rflag == 0:
                        self.Rrboxlist.append(
                            [self.mbbox[index + 2] - i, self.Lrboxlist[-1][-1]])
                        rflag = 1
                    if self.rmasks[self.mbbox[index + 1] + j, self.mbbox[index] + i, self.mbbox_rmasks_related_list[
                        k]] == True and lflag == 0:
                        self.Rlboxlist.append(
                            [self.mbbox[index] + i, self.mbbox[index + 1] + j])
                        lflag = 1
                    if self.rmasks[self.mbbox[index + 3] - j, self.mbbox[index + 2] - i, self.mbbox_rmasks_related_list[
                        k]] == True and rflag == 0:
                        self.Rrboxlist.append(
    ...
    def save(self):
        cv2.imwrite(saveDir+'\\'+self.name+'left4pos.png', self.left)
        cv2.imwrite(saveDir+'\\'+self.name+'right4pos.png', self.right)
    ...
    def std4pos(self):
        ...
        for i in range(l):
            ...
            for j in v:
                res = res + (j-avg)**2
            res = (res/l)**0.5
            #cove = res/avg #cove每个苹果的变异系数
            print(v)
            std.append(res)
            #cov.append(cove)
        return list_average(std)
...
def loadWeights(BALLOON_DIR, DEVICE, MODEL_DIR, config, ID,name):
...
def main(BALLOON_DIR, DEVICE, MODEL_DIR, config, ID, name,model,dataset):
    ...
    return [r['rois'], r['masks'],info['id']]




sample = process()
model,dataset=loadWeights(BALLOON_DIR,DEVICE,MODEL_DIR,config,0,'lmask')
stdLs = []
covLs = []
lpLs = []
for index in range(40):
    ...
    # RightImgWait4Test = cv2.imread(ls[0].replace('Left', 'Right'))
    # cv2.imwrite('D:/maskrcnn/Mask_RCNN-master/apple/val/Left.png', LeftImgWait4Test)
    # cv2.imwrite('D:/maskrcnn/Mask_RCNN-master/apple/val/Right.png', RightImgWait4Test)
    lbbox, lmasks, name = main(BALLOON_DIR, DEVICE, MODEL_DIR, config, index, 'lmask', model,dataset)
    z = np.zeros([1088,1920],np.uint8)
    print(name)
    for i in range(lmasks.shape[2]):
        # z = np.bitwise_or(lmasks[:,:,i],lmasks[:,:,i+1])
        for j in range(lmasks.shape[0]):
            for k in range(lmasks.shape[1]):
                if lmasks[j, k, i] == True:
                    z[j, k] = 255

    # input(lbbox.tolist())
    cv2.imwrite(r'D:\maskrcnn\maskrcnn\apple+logsYYY\apple\train\segmented/'+name[:name.find('.')] + '.jpg', z)
    with open(r'D:\maskrcnn\maskrcnn\apple+logsYYY\apple\train\segmented/'+name[:name.find('.')]+'.txt','w') as f:
        f.write(str(lbbox.tolist()))
    lbbox = lbbox.tolist()
    Lbbox = []
    for i in lbbox:
        for j in i:
    ...
    left = cv2.imread('D:/maskrcnn/Mask_RCNN-master/apple/val/Left.png')
    left2 = left
    right = cv2.imread('D:/maskrcnn/Mask_RCNN-master/apple/val/Right.png')
    right2 = right
    mbbox = imgmatch.match(left, right, lbbox)
    c = time.time()

    rbbox, rmasks,name = main(BALLOON_DIR, DEVICE, MODEL_DIR, config, 1, 'rmask',model,dataset)
    ...
    sample.reinit(left2, right2, lbbox, lmasks, mbbox,
                     rmasks, mbbox_rmasks_related_list,name)
    #a = time.time()
    sample.find4pos()
    #sample.point()
    #b = time.time()
    sample.locate()
    stdLs.append(sample.std4pos()*100)#stdLs保存的每项都代表一个图片中的平均标准差
    covLs.append(sample.cov4pos()*100)  # covLs保存的每项都代表一个图片中的变异系数
    lpLs.append(sample.lp4pos()*100)  # lpLs保存的每项都代表一个图片中的定位精度
    ...
    sample.show_xyz()
    # sample.point()
    APs=evaluate_model(dataset, model, config)
print('APs =' + str(APs))
print(stdLs)
print(covLs)
print(lpLs)
print('standard deviation='+str(list_average(stdLs)))
print('coefficient of variation='+str(list_average(covLs)))
print('localization precision='+str(list_average(lpLs)))
